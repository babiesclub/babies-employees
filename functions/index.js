...
- **groupsCount: כמעט תמיד 1**, אלא אם רשום במפורש בטקסט "X קבוצות" / "X קב'" (למשל "קב 2" בבאלון נפרד או בעמודה נפרדת). ברירת מחדל: 1.
- **פלוס "+" בשם הגן = שילוב של גנים באותה פעילות/קבוצה אחת** (לא כמה קבוצות!). לדוגמה "גן שקד+גן הדס" הוא ביקור אחד, קבוצה אחת (groupsCount=1), שם מלא כולל הפלוס.
- שם הגן הוא הטקסט המלא של הפעילות; אם יש מקף " - " או פסיק שמפריד כתובת → הכתובת אחריו.
  ...
  "entries": [
    {
      ...
      "note": "הערות נוספות בלבד — מספר קבוצות, חלוקה, הוראה מיוחדת, וגם ציון של 'לסרוגין' / 'שבוע א' / 'שבוע ב' / 'דו שבועי' (חשוב! זה מזוהה בצד לקוח). **אסור לכלול כתובת** — היא שייכת לגן. אם אין הערה אמיתית — החזירי null."
    }
  ],
  "warnings": []
}
...
exports.parsenetworkscheduleimage = onCall(
  ...
  async (req) => {
    ...
    const SYSTEM = `אתה מומחה לקריאת טבלאות שיבוץ שבועי של רשתות אפטר סקול / צהרונים בישראל. אתה מקבל תמונה של טבלה ומחזיר JSON מובנה.

🔴 חוק קריטי: **כל שורה בטבלה = סניף נפרד לחלוטין**. שני סניפים שונים לעולם לא חולקים שיבוץ — גם אם הכתובת דומה, גם אם איש הקשר זהה, גם אם התאים מודגשים באותו צהוב. **תא באזור צהוב בשורה N שייך אך ורק לסניף בשורה N** — לא לזה שמעליו ולא לזה שמתחתיו.

מבנה נפוץ של הטבלה:
- עמודה ימנית (הרחוקה ביותר) = שם הסניף + כתובת (רחוב, מספר, עיר). לפעמים גם שם השכבה/הכיתה.
- עמודה שאחריה = פרטי איש קשר (שם + טלפון)
- לפעמים בין העמודות יש עמודות נוספות עם שמות קבוצות/כיתות ("שקד", "רימון", "אלה", "חרצית", "צבר", "נרקיס", "צבעוני") — התעלמי מהן, הן שייכות למבנה הפנימי של הרשת ולא נחוצות לפלט.
- שאר העמודות = ימי השבוע (ראשון עד שישי) עם שעות ופעילויות בכל תא
- תא ריק ביום מסוים = אין פעילות באותו יום בסניף הזה. **אל תמציאי שיבוץ.**

פענוח תאי הימים:
- "החל מ HH:MM" או "החל מ־HH:MM" = שעת התחלה. אם לא מצוינת שעת סיום — הנח משך של שעה (start+60min).
- "HH:MM-HH:MM" = start-end במפורש
- הערות בתא (כמו "חיות עם נטלי (בייביז)", "בייביז חיות") מגדירות שהפעילות שייכת ל${asrTypeLbl}. אל תכניסי אותן ל-note אלא אם הן מוסיפות מידע ייחודי (כמו קבוצה מסוימת).
- **אם באותה עמודת יום, בשורה N יש N שיבוצים עוקבים באותה שעה** (למשל 4 תאים ברצף עם "13:00" בשורה של הסניף) → זה N קבוצות עוקבות של אותו סניף.
- **אבל אם באותה עמודת יום, השעות בשורות עוקבות שייכות לסניפים שונים** — הן שיבוצים של סניפים שונים, כל שיבוץ שייך לסניף בשורה שלו בלבד.
- **חוק זהב לפני שאת מוסיפה שיבוץ לסניף: איתור אנכי מדויק** — סמני מנטלית קו אנכי מתא השיבוץ עד עמודת שם הסניף בעמודה הימנית. הסניף באותה שורה = הבעלים של השיבוץ. **אל תשייכי שיבוץ לסניף בשורה שכנה גם אם כל השורה השכנה ריקה.**
